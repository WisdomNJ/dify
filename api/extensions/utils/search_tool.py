...
DB_CONFIG = {
    ...
    "database": dify_config.VANNA_DB_DATABASE,
    "user": dify_config.VANNA_DB_USERNAME,
    "password": dify_config.VANNA_DB_PASSWORD,
    "port": dify_config.VANNA_DB_PORT
}
# 创建5分钟过期时间的缓存
cache = TTLCache(maxsize=100, ttl=300)  # ttl=300秒=5分钟

@cached(cache)
def get_synonym_dict_by_type(type_:str,tenant_id:int = dify_config.SYS_TENANT_ID) -> dict[str, list[str]]:
    conn = None
    print(f"get_synonym_dict_by_type: {type_},tenant_id:{tenant_id}")
    try:
        # 连接PostgreSQL
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM flyway_synonyms where type_ = '{type_}' and tenant_id = {tenant_id}")
        ...
        # 将结果转换为字典列表，更方便使用
        dict_results = [dict(zip(col_names, row)) for row in results]
        results = {item["synonym_name"]:item['synonyms'].split(',') for item in dict_results}
        return results
...
def set_words(target_required:str,
    ...
    # 必填业务数据名词
    add_words_ner_by_key_words(required_words,2000,synonym_dict,tenant_id)
    # 非必填的业务数据名词
    un_required_words = target_un_required.split(",")
    add_words_ner_by_key_words(un_required_words,2000,synonym_dict,tenant_id)
...
def api_desc_match(question_text:str,
    ...
    # 设置名词
    set_words(target_required=target_required,target_un_required=target_un_required,word=word,synonym_dict=synonym_dict,tenant_id=tenant_id)
    # 判断是否匹配
